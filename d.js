 ...
 @namevi         TruyenCV downloader
 @namespace       httpdevs.forumvi.com
 @description     Tải truyện từ TruyenCV định dạng EPUB.
 @descriptionvi  Tải truyện từ TruyenCV định dạng EPUB.
 @version         4.6.8
 @icon            httpi.imgur.como5cmtkU.png
 @author          Zzbaivong
 @oujsauthor     baivong
 @license         MIT; httpsbaivong.mit-license.orglicense.txt
 @match           httptruyencv.com
 @match           httpstruyencv.com
 @require         httpscode.jquery.comjquery-3.5.1.min.js
 @require         httpsunpkg.comjszip@3.1.5distjszip.min.js
 @require         httpsunpkg.comfile-saver@2.0.2distFileSaver.min.js
 @require         httpsunpkg.comejs@2.7.4ejs.min.js
 @require         httpsunpkg.comjepub@2.1.4distjepub.min.js
 @require         httpsgreasemonkey.github.iogm4-polyfillgm4-polyfill.jsv=a834d46
 @noframes
 @connect         self
 @supportURL      httpsgithub.comlelinhtinhUserscriptissues
...
(function ($, window, document) {
  ...
  var converter = 'pshoan nghênh quảng đại bạn đọc quang lâmHuyền ảo khoái trí ân cừu';

  converter = new RegExp('(' + converter + ')', 'i');

  function cleanHtml(str) {
    str = str.replace(sChươngsd+s[^n], '');
    str = str.replace([^x09x0Ax0Dx20-uD7FFuE000-uFFFDu10000-u10FFFF]+gm, '');  eslint-disable-line
    str = str.replace(s[a-zA-Z0-9]{6,8}(=)sgm, function (key, attr) {
      if (attr) return ' ';
    ...
    str = str.replace(([^(]+button[^]+button[^)])sgi, '');
  ...
  function downloadError(mess, err) {
    ...
    if (errorAlert) errorAlert = confirm('Lỗi! ' + mess + 'nBạn có muốn tiếp tục nhận cảnh báo');

    return 'p class=no-indenta href=' + referrer + chapId + '' + mess + 'ap';
  }
  ...
  function saveEbook() {
    ...
    if (titleError.length) {
      titleError = 'p class=no-indentstrongCác chương lỗi strong' + titleError.join(', ') + 'p';
    } else {
      titleError = '';
    }
    beginEnd = 'p class=no-indentNội dung từ strong' + begin + 'strong đến strong' + end + 'strongp';
    ...
    $.ajax({
      ...
      .done(function (response) {
        var $data = $(response),
          ...
          $notContent = $chapter.find('iframe, script, style, a, div, phas(a[href=truyencv.com])'),
          ...
          if ($chapter.find('#btnChapterVip').length) {
            ...
            if ($img.length)
              $img.replaceWith(function () {
                return 'br a href=' + this.src + 'Click để xem ảnhabr ';
              });
  ...
  var pageName = document.title,
    ...
    credits =
      'pTruyện được tải từ a href=' +
      referrer +
      'TruyenCVappUserscript được viết bởi a href=httpslelinhtinh.github.iojEpubZzbaivongap',
    jepub;
