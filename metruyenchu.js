...
$(window).bind("load",function() {
	...
	try{
		var book_name = _x(xpath_book_name)[0].innerText;
		var book_author = _x(xpath_book_author)[0].innerText;
		var book_status = _x(xpath_book_status)[0].innerText;
		var book_dis = _x(xpath_book_dis)[0].innerHTML;
		var book_cov= _x(xpath_book_cov)[0];
		var book_info = {
			"book_name":book_name,
	...
	{
		var chap_next = _x(xpath_btn_next);
		if (chap_next.length<=0){
			var list_chap = GM_getValue("chaps",[]);
			alert("Count chap: "+list_chap.length);
			createEbook(GM_getValue("chaps",[]),GM_getValue("book_info",{}))
		}else{

			var chap_name = _x(xpath_chap_name)[0].innerText.replaceAll("<","_").replaceAll(">","_").replaceAll("\\","_").replaceAll("/","_");
			var chap_cont = _x(xpath_chap_cont)[0].innerHTML;
...
function createEbook(list_chap,book_info){
	var list_chap_temp = []
	for(var i =0;i<list_chap.length-1;i++){
		var insert = true;
		for(var j =0;i<list_chap_temp.length;j++){
	...
	for (i =0;i<list_chap.length;i++){
		...
		var chap_html = "<html xmlns='http://www.w3.org/1999/xhtml'>\n\
	 <head>\n\
	...
	var toc_file = "<?xml version='1.0' encoding='utf-8'?>\n\
	...
	var content_file = "<?xml version='1.0' encoding='utf-8'?>\n\
